...
SEXP docMatrixDF(SEXP charVec, SEXP charLeng, SEXP pos, SEXP posN, SEXP minFreq,
  ...
  PROTECT(charLeng = AS_INTEGER(charLeng));
  pc++;
  int file_num = INTEGER_POINTER(charLeng)[0];
  PROTECT(charVec = AS_CHARACTER(charVec));
  pc++;

  map<string, int> ma0;  //, ma[file_num];     //
                         //全要素用のマップと要素ごとのマップ // 2009 04 03
  ...
  // map<string, int> ma0, ma[file_num];     //
  // 全要素用のマップと要素ごとのマップ map<string, int>::iterator pma0, pma;//
  // マップ検索用

  for (file = 0; file < file_num; file++) {
    //	  Rprintf("Row %d check\n", file);
    if (strlen(CHAR(STRING_ELT(charVec, file))) < 1 ||
        STRING_ELT(charVec, file) == NA_STRING) {
      // 		Rprintf("in ISNA\n");
      //  		ma[file].insert(make_pair("NA", 0));// 1 は
      //  1個目と言う意
    ...
    for (; node; node = node->next) {
      ...
      if (node->stat == MECAB_BOS_NODE)
        ...
        // 		  buf1[node->length] = '\0';// 末尾にNULLを加える// 2006
        // 06 移動
          ...
          if (j == 1) {  // 品詞情報1
            ...
            for (z = 0; z < pos_n; z++) {
              sprintf(
                  buf4, "%s",
                  Ppos2[z]
                      .c_str());  // 2011 03 10 // sprintf(buf4, "%s", Ppos[z]);
              sprintf(buf3, "%s", p);
              // Rprintf("buf3 %s\n", buf3);
            ...
            pma0 =
                ma0.find(str);  // 出てきた形態素原型は既に全体マップにあるか？
            if (pma0 != ma0.end()) {
              //				Rprintf("incremnet\n");
            ...
            // pma =
            // ma[file].find(str);//出てきた形態素原型は既に個別マップにあるか？
            ...
            if (pma != (vecmap.at(file)).end()) {  // if(pma != ma[file].end()){
              ...
              (vecmap.at(file))
                  .insert(make_pair(str, 1));  // ma[file].insert(make_pair(str,
                                               // 1));// 1 は 1個目と言う意味
            }
          }
          p = strtok(NULL, ",");
  ...
  PROTECT(ans = allocMatrix(INTSXP, file_num, n0));
  pc++;  // docMatrixDF
  rans = INTEGER(ans);

  // 各要素から取得したマップを，全要素ファイルから取得したマップ
  ...
  Rprintf("\n");  // 2006 03 27
                  // 行名のセット// これは要素番号
  PROTECT(row_names = allocVector(STRSXP, file_num));
  pc++;
  i = 0;

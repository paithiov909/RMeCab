...
SEXP docNgramDF(SEXP mojiVec, SEXP rowN, SEXP type, SEXP pos, SEXP posN,
  ...
  PROTECT(rowN = AS_INTEGER(rowN));
  pc++;
  int row_num = INTEGER_POINTER(rowN)[0];
  PROTECT(mojiVec = AS_CHARACTER(mojiVec));
  pc++;

  // 	for(file = 0; file < row_num; file ++){
  // 	 f[file]  =  R_alloc(strlen(CHAR(STRING_ELT(mojiVec, file))),
  // sizeof(char));
  //         }
  // 	for(file = 0; file < row_num; file ++){
  ...
  if (pos_n > 0 && flag) {
    ...
    for (i = 0; i < pos_n; i++) {
      ...
      if (strcmp(Ppos2[i].c_str(), KIGO) ==
          0) {  // if(strcmp(Ppos[i], KIGO) == 0){// 2011 03 10
        mSym = 1;
      }
    }
  }  // 2005 06 23
  ...
  for (file = 0; file < row_num; file++) {
    ...
    // Rprintf("file = %d\n", file);
    if (strlen(CHAR(STRING_ELT(mojiVec, file))) < 1 ||
        STRING_ELT(mojiVec, file) == NA_STRING) {
      // Rprintf("in ISNA\n");
      continue;
      // 		Rprintf("in ISNA\n");
    ...
    if (typeSet == 0) {  // 文字単位なら
      ...
      if (strlen(input) > 0) {
        ...
        for (wz = 0; wz < wcslen(wbuf); wz++) {  // 2005 07 22
          ...
          // #if defined(WIN32)
          // 			wsprintf(target2, "%lc", wbuf[wz]);// windows
          // では wsprintf #elif  defined(__MINGW32__) 			wsprintf(target2, "%lc",
          // wbuf[wz]);//  windows では wsprintf #else 			sprintf(target2, "%lc",
          // wbuf[wz]);// Linux  では sprintf #endif 2008 04 05 #if
          ...
          if (strcmp((char *)target2, " ") == 0 ||
            ...
            if (strL.size() >= (unsigned int)Ngram) {
              ...
              if (pma !=
                ...
                (vecmap.at(file))
                    .insert(
                        make_pair(target, 1));  // ma[file].insert(make_pair(target,
                                                // 1));// 1 は 1個目と言う意味
              }
      ...
      /// 解析結果のノードをなめる
      for (; node;
           node = node->next) {  // node とはその文の形態素ごとに設定される
        //			  		printf("%d ", node->id);
            ...
            if (j == 1 && p != NULL) {
              ...
              for (i = 0; i < pos_n; i++) {
                sprintf(
                    buf4, "%s",
                    Ppos2[i]
                        .c_str());  // 2011 03 10 sprintf(buf4, "%s", Ppos[i]);
                //					Rprintf("buf4 %s\n",
                //buf4);
        ...
        if (strL.size() >=
          ...
          target.append("]");
          pma0 =
              ma0.find(target);  // 出てきた形態素原型は既に全体マップにあるか？
          if (pma0 != ma0.end()) {
          ...
          pma =
              (vecmap.at(file))
                  .find(
                      target);  // ma[file].find(target );// str 2006 08 25
          ...
          if (pma != (vecmap.at(file)).end()) {  // if(pma != ma[file].end()){
            ...
            (vecmap.at(file))
                .insert(make_pair(target, 1));  // ma[file].insert(make_pair(target,
                                                // 1));// 1 は 1個目と言う意味
          }
  ...
  PROTECT(ans = allocMatrix(INTSXP, row_num, n0));
  pc++;  // docNgram2
  rans = INTEGER(ans);
  ...
  // 行名のセット// これはファイル名
  // pv = v.begin();
  PROTECT(row_names = allocVector(STRSXP, row_num));
  pc++;
  for (file = 0; file < row_num; file++) {
    //	  Rprintf("row names = %s\n", f[file]);

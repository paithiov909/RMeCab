...
SEXP NgramDF2(SEXP directory, SEXP origF, SEXP fileN, SEXP ft, SEXP type,
  ...
  // 	 f[file]  =  R_alloc(strlen(CHAR(STRING_ELT(origF, file))),
  // sizeof(char));
  //  }
  // 	for(file = 0; file < f_count; file ++){
  ...
  wchar_t wbuf[BUF4];  // = { 0 }; //wchar_t  wbuf [5120] = { 0 }; /*
                       // ワイド文字列 : 日本語文字数  + 1 */
  ...
  for (file = 0; file < f_count; file++) {
    ...
    sprintf(file_name, "%s/%s", path,
            ff[file].c_str());  // sprintf(file_name, "%s/%s", path, f[file]);
    ...
    if ((fp = fopen(file_name, "r")) == NULL) {
      ...
      while (!feof(fp)) {
        if (fgets(input, FILEINPUT, fp) !=
            NULL) {  // 2010 03 11  if(fgets(input, 5120, fp) != NULL){
          if (strlen(input) < 1) {
            continue;
          }
...
sprintf(target2, "%lc", wbuf[wz]);// Linux  では sprintf
                ...
                */
                ...
                // strcpy(buf1, *target2);
                if (*target2 > 0x00 &&
                    *target2 < 0x21) {  // エスケープ記号類0x0e
                  continue;
                }  //
                /////////////// windows では wsprintf(str[ys], "%lc", wbuf[z+ys
                ///+ yw]);
                ...
                if (strcmp((char *)target2, " ") == 0 ||
                  ...
                  if (strL.size() >= (unsigned int)Ngram) {
                    ...
                    pma =
                        (vecmap.at(file))
                            .find(
                                target);  // ma[file].find(target);//出てきた形態素原型は既に個別マップにあるか？
                    if (pma != (vecmap.at(file))
                                   .end()) {  // if(pma != ma[file].end()){
                      pma->second = pma->second + 1;
                      // 二つ目の数値を加算
                    } else {  // マップにないなら，新規にマップに追加
                      (vecmap.at(file))
                          .insert(make_pair(target,
                                            1));  // ma[file].insert(make_pair(target,
                                                  // 1));// 1 は 1個目と言う意味
                    }
            ...
            for (; node;
                 node =
                     node->next) {  // node とはその文の形態素ごとに設定される
              //			  		printf("%d ", node->id);
              ...
              if (node->stat == MECAB_BOS_NODE)
                ...
                // if( atoi(buf1) >  0x00 &&  atoi(buf1) < 0x0e ){// if(
                // atoi(buf1)  == 0x0e){//エスケープ記号類
                ...
                if (typeSet == 2) {           // 品詞情報で数える
                  ...
                  if (j == 1 && p != NULL) {
                    ...
                    for (i = 0; i < pos_n; i++) {
                      sprintf(
                          buf4, "%s",
                          Ppos2[i].c_str());  // sprintf(buf4, "%s", Ppos[i]);
                      //					Rprintf("buf4
                      //%s\n", buf4);
              ...
              if (strL.size() >=
                  (unsigned int)
                      Ngram) {  // リストのサイズが指定通りであるなら，保存を始める
                //				  Rprintf("type = %d, strL size
                //=  %d\n", typeSet, strL.size() );
                ...
                for (iter = strL.begin(); iter != strL.end(); iter++) {
                  ...
                  target.append(
                      *iter);  // target.append( buf3); //target.append( *iter);
                ...
                pma =
                    (vecmap.at(file))
                        .find(
                            target);  // ma[file].find(target);//str
                                      // 出てきた形態素原型は既に個別マップにあるか？
                if (pma !=
  ...
  while (pma0 != ma0.end()) {
    if (pma0->second < mFreq) {
      ma0.erase(pma0++);  /////	ma0.erase(pma0);// 2007 09 15 //
                          ///ma0.erase(pma0++);
    } else {
      ++pma0;
    }
  ...
  // データフレームの生成
  //		Rprintf("nn = %d\n", nn);
  //	PROTECT(mydf = allocVector(VECSXP, Ngram+1));pc++;//Ngram +  頻度列
  //のdata.frame
  PROTECT(mydf = allocVector(VECSXP, Ngram + f_count));
  pc++;  // Ngram +  ファイル数のdata.frame
  for (xx = 0; xx < Ngram; xx++) {
    SET_VECTOR_ELT(mydf, xx,
                   allocVector(STRSXP, n0));  // 各Ngram 列のメモリ n0 行分用意
  }
  //	SET_VECTOR_ELT(mydf, Ngram, allocVector(INTSXP, n0));// 頻度列のメモリ
  //n0 行分を用意
  for (xx = Ngram; xx < Ngram + f_count; xx++) {
    SET_VECTOR_ELT(mydf, xx,
                   allocVector(INTSXP, n0));  // 頻度列のメモリ n0 行分を用意
  }
  ...
  for (xx = 0; xx < n0; xx++) {  // n0 個のタームの数だけ繰り返す
    ...
    while (p != NULL) {
      ...
      //  // < 2005 11 08>
      // #if defined(WIN32)
      // 		SET_VECTOR_ELT(VECTOR_ELT(mydf, j++), xx, mkCharCE( buf3
      // , CE_NATIVE)); #elif  defined(__MINGW32__)
      // 		SET_VECTOR_ELT(VECTOR_ELT(mydf, j++), xx, mkCharCE( buf3
      // , CE_NATIVE)); #else 		SET_VECTOR_ELT(VECTOR_ELT(mydf, j++), xx,
      // mkCharCE( buf3, CE_UTF8 )); #endif
      // 		// SET_VECTOR_ELT(VECTOR_ELT(mydf, j++), xx, mkChar(
      // buf3 ));//j列のxx行にセット
      //   // </ 2005 11 08>
  ...
  for (file = 0; file < f_count; file++) {
    ...
    while (j < n0 && pma0 != ma0.end()) {
      ...
      if (pma !=
        ...
        INTEGER(VECTOR_ELT(mydf, Ngram + file))
        [j] = pma->second;  // Ngram 列の最後に頻度情報
      } else {
  ...
  // 	  //df 内ベクトルの名前を用意
  //	PROTECT(varlabels = allocVector(STRSXP, Ngram+1)); pc++;
  PROTECT(varlabels = allocVector(STRSXP, Ngram + f_count));
  pc++;
  //  その単純な初期化
  for (xx = 0; xx < Ngram; xx++) {
    sprintf(buf3, "Ngram%d", xx + 1);
    ...
    SET_STRING_ELT(
        ...
        mkCharCE(ff[j++].c_str(),
                 CE_UTF8));  // (utf8locale)?CE_UTF8:CE_NATIVE));// 2011 03 10
                             // SET_STRING_ELT(varlabels, xx, mkCharCE(f[j++],

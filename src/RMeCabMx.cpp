...
SEXP RMeCabMx(SEXP filename, SEXP pos, SEXP posN, SEXP minFreq, SEXP kigo,
  ...
  if (pos_n > 0 && flag) {
    ...
    // Ppos[i] = R_alloc( (unsigned int ) strlen (CHAR(STRING_ELT(myPos, i))),
    // sizeof(char));
    // }
    //   Rprintf("end myPos = AS_CHARACTER(pos) \n");
    for (i = 0; i < pos_n; i++) {
      // 2011 03 10 strcpy(Ppos[i], CHAR(STRING_ELT(myPos, i)));
      Ppos2.push_back(CHAR(
          STRING_ELT(myPos, i)));  // 2011 03 10
                                   //	Rprintf("Pos[%d] =  %s\n", i, Ppos[i]);
    ...
    // 2011 03 10 // Ppos[0] = R_alloc(  (unsigned int )  strlen( meisiCode()),
    // sizeof(char)); 2011 03 10 // strcpy(Ppos[0], meisiCode() );
    Ppos2.push_back(meisiCode());  // 2011 03 10
    // 2011 03 10 //  Ppos[1] = R_alloc( (unsigned int )  strlen( keiyouCode()
    // ), sizeof(char)); 2011 03 10 //  strcpy(Ppos[1], keiyouCode() );
  ...
  if ((fp = fopen(input, "r")) == NULL) {
        ...
        for (; node; node = node->next) {
          ...
          if (node->stat == MECAB_BOS_NODE)
            ...
            if (p != NULL) {
              ...
              for (i = 0; i < pos_n; i++) {
                sprintf(buf4, "%s",
                        Ppos2[i].c_str());  // 2011 03 10 // sprintf(buf4, "%s",
                                            // Ppos[i]);
                //				Rprintf("buf4 %s\n", buf4);
    ...
    if (n < 1) {
      ...
      // #endif
      // 	  //  SET_STRING_ELT(vecName, 0, mkChar(buf3));  //
      // 規程頻度以下のトークン数
      //  // </ 2005 11 08>
      ...
      //  // < 2005 11 08>
      // #if defined(WIN32)
      // 	  SET_STRING_ELT(vecName, 1, mkCharCE( "[[TOTAL-TOKENS]]",
      // CE_NATIVE )); #elif  defined(__MINGW32__) 	  SET_STRING_ELT(vecName, 1,
      // mkCharCE( "[[TOTAL-TOKENS]]", CE_NATIVE )); #else
      // 	  SET_STRING_ELT(vecName, 1, mkCharCE( "[[TOTAL-TOKENS]]",
      // CE_UTF8 )); #endif
      // 			// SET_STRING_ELT(vecName, 1, mkChar(
      // "[[TOTAL-TOKENS]]" ));
      //  // </ 2005 11 08>

      SET_STRING_ELT(vecName, 1,
                     mkCharCE("[[TOTAL-TOKENS]]",
    ...
    for (z = 0; z < n; z++) {
      ...
      // mkChar((pa->first).c_str()));
      // // 	  INTEGER(VECTOR_ELT(mydf,1))[z] =   pa->second;//
      // 最後に頻度情報
      ...
      if ((pa->second) < mFreq) {
        ...
        //  // < 2005 11 08>
        // #if defined(WIN32)
        // 		SET_STRING_ELT(vecName, zz,
        // mkCharCE((pa->first).c_str(), CE_NATIVE )); #elif
        // defined(__MINGW32__) 		SET_STRING_ELT(vecName, zz,
        // mkCharCE((pa->first).c_str() , CE_NATIVE)); #else
        // 		SET_STRING_ELT(vecName, zz, mkCharCE((pa->first).c_str()
        // , CE_UTF8)); #endif
        // 		//SET_STRING_ELT(vecName, zz, mkChar((pa->first).c_str()
        // ));  // 形態素情報をセット
        // // </ 2005 11 08>

        SET_STRING_ELT(
            vecName, zz,
            mkCharCE((pa->first).c_str(),
                     CE_UTF8));  // (utf8locale)?CE_UTF8:CE_NATIVE ));
    ...
    if (zz < 1) {
      ...
      //  // < 2005 11 08>
      // #if defined(WIN32)
      // 		 SET_STRING_ELT(vecName, zz  , mkCharCE(buf3,
      // CE_NATIVE)); #elif  defined(__MINGW32__) 		 SET_STRING_ELT(vecName, zz  ,
      // mkCharCE(buf3, CE_NATIVE)); #else 		 SET_STRING_ELT(vecName, zz  ,
      // mkCharCE(buf3, CE_UTF8)); #endif
      // 		 //SET_STRING_ELT(vecName, zz  , mkChar(buf3));  //
      // 規程頻度以下のトークン数
      //  // </ 2005 11 08>
      ...
      //  // < 2005 11 08>
      // #if defined(WIN32)
      // 	 SET_STRING_ELT(vecName, zz+1  , mkCharCE(  "[[TOTAL-TOKENS]]",
      // CE_NATIVE  )); #elif  defined(__MINGW32__) 	 SET_STRING_ELT(vecName, zz+1
      // , mkCharCE(  "[[TOTAL-TOKENS]]", CE_NATIVE  )); #else
      // 	 SET_STRING_ELT(vecName, zz+1  , mkCharCE(  "[[TOTAL-TOKENS]]",
      // CE_UTF8  )); #endif
      // 	 // SET_STRING_ELT(vecName, zz+1  , mkChar(  "[[TOTAL-TOKENS]]"
      // ));  //// テキスト総トークン数をセット
      //  // </ 2005 11 08>

      SET_STRING_ELT(vecName, zz + 1,
                     mkCharCE("[[TOTAL-TOKENS]]",
                              CE_UTF8));  // (utf8locale)?CE_UTF8:CE_NATIVE  ));
      ...
      // #endif
      // 	//SET_STRING_ELT(vecName, zz-1 , mkChar(buf3));  //
      // 規程頻度以下のトークン数
      //   // </ 2005 11 08>
      ...
      //  // < 2005 11 08>
      // #if defined(WIN32)
      // 		SET_STRING_ELT(vecName, zz , mkCharCE(
      // "[[TOTAL-TOKENS]]", CE_NATIVE  )); #elif  defined(__MINGW32__)
      // 		SET_STRING_ELT(vecName, zz , mkCharCE(
      // "[[TOTAL-TOKENS]]", CE_NATIVE  )); #else 		SET_STRING_ELT(vecName, zz ,
      // mkCharCE(  "[[TOTAL-TOKENS]]", CE_UTF8 )); #endif
      // 		//SET_STRING_ELT(vecName, zz , mkChar(
      // "[[TOTAL-TOKENS]]"  ));  //// テキスト総トークン数をセット
      //  // </ 2005 11 08>

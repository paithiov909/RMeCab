        ...
        if (typeSet == 0) {  // 文字単位なら
          ...
          if (strlen(input) > 0) {
            ...
            for (wz = 0; wz < wcslen(wbuf); wz++) {  // 2005 07 22
              ...
              // defined(_WIN64) 		wsprintf(target2, "%lc", wbuf[wz]);// windows
              // では wsprintf #elif  defined(__MINGW32__) ||
              // defined(__WINGW64__) 		wsprintf(target2, "%lc", wbuf[wz]);//
              // windows では wsprintf #else 				sprintf(target2, "%lc",
              // wbuf[wz]);// Linux  では sprintf #endif

              // 2008 04 05 #if defined(_WIN64) || !defined(_WIN32)
              // defined(__MINGW32__) || defined(__MINGW64__
              /* 2022 04 30
#if defined(WIN32) || defined(WIN64) || defined(_WIN32) || defined(_WIN64) //
2022 04 30 wsprintf(target2, "%lc", wbuf[wz]);// windows では wsprintf #elif
defined(__MINGW32__) || defined(__WINGW64__) wsprintf(target2, "%lc",
wbuf[wz]);//  windows では wsprintf #else sprintf(target2, "%lc", wbuf[wz]);//
Linux  では sprintf #endif // 2022 04 30
              */
              ...
              // 				//< 2005 11 08>
              // 				// if( atoi(target2) >  0x00 &&
              // atoi(target2) < 0x0e ){//if( atoi(buf1) ==
              // 0x0e){//エスケープ記号類 				if( target2[0] >  0x00 &&  target2[0]
              // < 0x21 ){//if( atoi(buf1) == 0x0e) 				  continue;
              // 				}// </ 2005 11 08>
    ...
    if (n > OVERLINE) {                   // 40000 -> OVERLINE  // 2016 12 27
      ...
      for (z = 0; z < nn; z++) {
        //  // < 2005 11 08>
        // #if defined(WIN32)
        // 	SET_VECTOR_ELT(VECTOR_ELT(mydf, 0), z, mkCharCE(
        // (pa->first).c_str(), CE_NATIVE )); #elif  defined(__MINGW32__)
        // 	SET_VECTOR_ELT(VECTOR_ELT(mydf, 0), z, mkCharCE(
        // (pa->first).c_str(), CE_NATIVE )); #else
        // 	SET_VECTOR_ELT(VECTOR_ELT(mydf, 0), z, mkCharCE(
        // (pa->first).c_str(), CE_UTF8 )); #endif
      ...
      // // < 2005 11 08>
      // #if defined(WIN32)
      // 		SET_STRING_ELT(varlabels, 0, mkCharCE("Ngram",
      // CE_NATIVE)); 		SET_STRING_ELT(varlabels, 1, mkCharCE("Freq", CE_NATIVE));
      // #elif  defined(__MINGW32__)
      // 		SET_STRING_ELT(varlabels, 0, mkCharCE("Ngram",
      // CE_NATIVE)); 		SET_STRING_ELT(varlabels, 1, mkCharCE("Freq", CE_NATIVE));
      // #else
      // 		SET_STRING_ELT(varlabels, 0, mkCharCE("Ngram",
      // CE_UTF8)); 		SET_STRING_ELT(varlabels, 1, mkCharCE("Freq", CE_UTF8));
      // #endif

...
SEXP RMeCabC(SEXP str, SEXP mypref, SEXP mydic) {
  ...
  const char *dic =
      CHAR(STRING_ELT(mydic, 0));  // 指定オプション(辞書だけではない)
  ...
  PROTECT(my_list = allocVector(VECSXP, 5000));
  pa++;  // pa++; 1//2019 06 10 1000->5000
  ...
  for (; node; node = node->next) {
    if (node->stat == MECAB_BOS_NODE)
      ...
      // // 品詞情報をセット #else
      // 		  // Rprintf("CE_UTF8\n");
      // 		SET_STRING_ELT(my_char2, 0, mkCharCE(p, CE_UTF8));  //
      // 品詞情報をセット #endif
      ...
      if (pref == 1) {  // 原型を取るため，六個目の要素を取る
        ...
        if (p == NULL || strcmp(p, "*") == 0) {
          // 			  // < 2005 11 08>
          // #if defined(WIN32)
          // 			  SET_STRING_ELT(my_char, 0, mkCharCE(buf1,
          // CE_NATIVE)); #elif  defined(__MINGW32__) 			  SET_STRING_ELT(my_char, 0,
          // mkCharCE(buf1, CE_NATIVE));  // 形態素情報をセット #else
          // 			  // Rprintf("CE_UTF8\n");
          // 			  SET_STRING_ELT(my_char, 0, mkCharCE(buf1,
          // CE_UTF8));  // 形態素情報をセット #endif
          // 			  // SET_STRING_ELT(my_char, 0,
          // mkChar(buf1));//もとの語形をセット
          // 			  // </ 2005 11 08>
          ...
          // < 2005 11 08>
          // #if defined(WIN32)
          // 			  SET_STRING_ELT(my_char, 0, mkCharCE(p,
          // CE_NATIVE)); #elif  defined(__MINGW32__) 			  SET_STRING_ELT(my_char, 0,
          // mkCharCE(p, CE_NATIVE)); #else
          // 			  //Rprintf("CE_UTF8\n");
          // 			  SET_STRING_ELT(my_char, 0, mkCharCE(p,
          // CE_UTF8)); #endif
          // 			  // SET_STRING_ELT(my_char, 0,
        ...
        // 			SET_STRING_ELT(my_char, 0, mkCharCE(buf1,
        // CE_NATIVE)); #elif  defined(__MINGW32__) 			SET_STRING_ELT(my_char, 0,
        // mkCharCE(buf1, CE_NATIVE)); #else 			  SET_STRING_ELT(my_char, 0,
        // mkCharCE(buf1, CE_UTF8)); #endif
        // 			// SET_STRING_ELT(my_char, 0,
      ...
      // //もと単語の名前に形態素情報 UNPROTECT(2);// pa = pa - 1; 1
      SET_VECTOR_ELT(my_list, i, my_char);  // リストに追加
      UNPROTECT(2);
      pa--;
      pa--;  // pa = pa - 1; 1
      i++;
      if (i + 5000 > OVERLINE) {  // リストが20万（四万）を越えるならエラーを起こしやすい//2019
                                  // 06 10 1000-> 5000
        Rprintf("i == %d stop\n", i);
        mecab_destroy(mecab);
      ...
      if (xlength(my_list) <=
        ...
        UNPROTECT(1);
        pa--;
        PROTECT(my_list = lengthgets(my_list, i + 5000));
        pa++;  // pa++;  1//2019 06 10 1000->5000
      }
      p = NULL;
  ...
  pa--;  // pa--;0
  if (xlength(my_list) >
      (R_xlen_t)i) {  // 2015 12 18 //  if(length(my_list) > i){
    PROTECT(my_list = lengthgets(my_list, i));
    pa++;  // pa++;1
  }
  UNPROTECT(pa);  //   UNPROTECT(1);   // pa--;                               //
                  //   pa--;0

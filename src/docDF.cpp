...
SEXP docDF(SEXP directory, SEXP origF, SEXP fileN, SEXP ft, SEXP type, SEXP pos,
           SEXP posN, SEXP minFreq, SEXP N,
           // 			 SEXP sym,
           // 			 SEXP kigo,
           SEXP Genkei, SEXP nDF, SEXP mydic) {
  // Rprintf("BUF1  =  %i\n", BUF1); //
  // Rprintf("BUF2  =  %i\n", BUF2); //
  // Rprintf("BUF3  =  %i\n", BUF3); //
  // Rprintf("BUF4  =  %i\n", BUF4); //
  ...
  SEXP tmp, row_names,
      mydf = R_NilValue,
      varlabels =
          R_NilValue;  // 2011 03 11 //  SEXP mydf, tmp, row_names, varlabels;//
                       // SEXP ans, dim, dimnames, row_names, col_names;

  int mFt =
      INTEGER_VALUE(ft);  // ファイル 0 かディレクトリ 1 かデータフレーム列か2
  /////

  FILE* fp;  // 2009 04 03
  map<string, int>
      ma0;  //, ma[f_count];     // ファイル数の数+登録単語用マップの数1
  ...
  if (mFt == 1 || mFt == 0) {  // ファイル 0 かディレクトリ 1
    path = R_alloc(strlen(CHAR(STRING_ELT(directory, 0))),
                   sizeof(char));  // ディレクトリ名
  ...
  if (pos_n > 0) {  // if(flag){//if(pos_n > 0){}
    ...
    // )); 	  Ppos[0] = R_alloc(strlen(CHAR(STRING_ELT(myPos, 0))),
    // sizeof(char)); 	  strcpy(Ppos[0], CHAR(STRING_ELT(myPos, 0)));
  }
  ...
  // pma;// マップ検索用 list <string> hinsi, strL, saibun; list
  // <string>::iterator hinsi_it, iter, saibun_it;
  ...
  for (file = 0; file < f_count; file++) {
    ...
    if (mFt == 2) {  // データフレームのベクトル
      ...
      if (strlen(CHAR(STRING_ELT(origF, file))) < 1 ||
          STRING_ELT(origF, file) == NA_STRING) {
        // 		Rprintf("in ISNA\n");
        continue;
      }
      // input = (char []) R_alloc(strlen(CHAR(STRING_ELT(directory, file))),
      // sizeof(char));
      ...
      // setMeCabMap(typeSet, input, ma0,  ma[file], pma0, pma, strL, iter,
      // hinsi, hinsi_it, saibun, saibun_it,  Ppos, pos_n, mSym,  KIGO, Ngram,
      // genkei);
      setMeCabMap(typeSet, input, ma0, vecmap.at(file), pma0, pma, strL, iter,
                  hinsi, hinsi_it, saibun, saibun_it, Ppos2, pos_n, Ngram,
                  genkei, dic);
      ...
      if ((fp = fopen(file_name, "r")) == NULL) {
        ...
        while (!feof(fp)) {
          ...
          if (fgets(input, FILEINPUT, fp) !=
            ...
            // setMeCabMap(typeSet, input, ma0,  ma[file], pma0, pma, strL,
            // iter,  hinsi, hinsi_it, saibun, saibun_it, Ppos, pos_n, mSym,
            // KIGO, Ngram, genkei);
            setMeCabMap(typeSet, input, ma0, vecmap.at(file), pma0, pma, strL,
                        iter, hinsi, hinsi_it, saibun, saibun_it, Ppos2, pos_n,
                        Ngram, genkei, dic);
            ////////////////////////////////////////////////
          }
        }  // while(feop)
  ...
  if (typeSet == 0 || typeSet == 2) {
    PROTECT(mydf = allocVector(VECSXP, 1 + f_count));
    pc++;
    ...
    if (NDF == 1) {  // 名詞組を独立したデータフレーム列として返す場合
      i = Ngram + 2 + f_count;
      PROTECT(mydf = allocVector(VECSXP, i));
      pc++;
      for (j = 0; j < i; j++) {
        if (j < Ngram + 2) {
      ...
      PROTECT(mydf = allocVector(VECSXP, 3 + f_count));
      pc++;  // 名詞組-品詞組ｰ再分類1 +  ファイル数のdata.frame   // 列数
  ...
  if (typeSet == 0) {  // 文字の場合
    ...
    for (xx = 0; xx < n0 && pma0 != ma0.end();
      ...
      // #if defined(WIN32)
      // 			SET_VECTOR_ELT(VECTOR_ELT(mydf, 0), xx,
      ...
      for (file = 0; file < f_count && pma0 != ma0.end(); file++) {
        ...
        if (pma != (vecmap.at(file))
                       .end()) {  // if(pma != ma[file].end()){// 見つかった
          INTEGER(VECTOR_ELT(mydf, 1 + file))
          [xx] = pma->second;  // 新規列に追加
        } else {
    ...
    for (xx = 0; xx < n0; xx++) {           // n0 行のタームの数だけ繰り返す
      ...
      while (p != NULL) {  // _TYPE_1
        ...
        if (NDF == 1 && i <= Ngram) {  // タームはデータフレーム形式で
          ...
          // #if defined(WIN32)
          // 				SET_VECTOR_ELT(VECTOR_ELT(mydf, j++),
          // xx, mkCharCE( buf3, CE_NATIVE  ));//j列のxx行にセット #elif
          // defined(__MINGW32__)
          // SET_VECTOR_ELT(VECTOR_ELT(mydf, j++), xx, mkCharCE( buf3, CE_NATIVE
          // )); #else 			  SET_VECTOR_ELT(VECTOR_ELT(mydf, j++),
          // xx, mkCharCE( buf3, CE_UTF8  )); #endif

          SET_STRING_ELT(
              VECTOR_ELT(mydf, j++), xx,
        ...
        if ((i % Ngram) == 0) {
          ...
          // #if defined(WIN32)
          // 			  SET_VECTOR_ELT(VECTOR_ELT(mydf, j++), xx,
          // mkCharCE( buf3, CE_NATIVE  ));//j列のxx行にセット #elif
          // defined(__MINGW32__)
          // SET_VECTOR_ELT(VECTOR_ELT(mydf, j++), xx, mkCharCE( buf3, CE_NATIVE
          // )); #else 			  SET_VECTOR_ELT(VECTOR_ELT(mydf, j++),
          // xx, mkCharCE( buf3, CE_UTF8  )); #endif
        ...
        // // // #if defined(WIN32)
        // // // 			  SET_VECTOR_ELT(VECTOR_ELT(mydf, j++),
        // xx, mkCharCE( buf3, CE_NATIVE  ));//j列のxx行にセット
        // // // #elif  defined(__MINGW32__)
        // // // 			  SET_VECTOR_ELT(VECTOR_ELT(mydf, j++),
        // xx, mkCharCE( buf3, CE_NATIVE ));
        // // // #else
        // // // 			  SET_VECTOR_ELT(VECTOR_ELT(mydf, j++),
        // xx, mkCharCE( buf3, CE_UTF8  ));
        // // // #endif
      ...
      for (file = 0; file < f_count && pma0 != ma0.end(); file++) {
        ...
        if (pma != (vecmap.at(file))
                       .end()) {  // if(pma != ma[file].end()){// 見つかった
          if (NDF == 1) {
            INTEGER(VECTOR_ELT(mydf, Ngram + 2 + file))
            [xx] = pma->second;  // 新規列に追加
          } else {
  ...
  if (typeSet == 0) {  // 文字グラムの場合
    PROTECT(varlabels = allocVector(STRSXP, 1 + f_count));
    pc++;
    ...
    SET_STRING_ELT(
        varlabels, 0,
        mkCharCE("Ngram", CE_UTF8));  // (utf8locale)?CE_UTF8:CE_NATIVE ));
    ...
    for (j = 0; j < f_count; j++) {
      if (mFt == 2) {                   // データフレームの場合
        ...
        // #if defined(WIN32)
        // 		  SET_STRING_ELT(varlabels, j+1, mkCharCE(buf4,
        // CE_NATIVE )); #elif  defined(__MINGW32__)
        // SET_STRING_ELT(varlabels, j+1, mkCharCE(buf4, CE_NATIVE )); #else
        // SET_STRING_ELT(varlabels, j+1, mkCharCE(buf4, CE_UTF8 )); #endif

        SET_STRING_ELT(
            varlabels, j + 1,
            mkCharCE(buf4, CE_UTF8));  // (utf8locale)?CE_UTF8:CE_NATIVE ));

      } else {  // ファイルの場合

        // #if defined(WIN32)
        // 		  SET_STRING_ELT(varlabels, j+1, mkCharCE(f[j],
        // CE_NATIVE )); #elif  defined(__MINGW32__)
        // SET_STRING_ELT(varlabels, j+1, mkCharCE(f[j], CE_NATIVE )); #else
        // SET_STRING_ELT(varlabels, j+1, mkCharCE(f[j], CE_UTF8 )); #endif

        SET_STRING_ELT(
            varlabels, j + 1,
            mkCharCE(
                ff[j].c_str(),
                CE_UTF8));  // (utf8locale)?CE_UTF8:CE_NATIVE )); // 2011 03 10
                            // SET_STRING_ELT(varlabels, j+1, mkCharCE(f[j],
    ...
    if (NDF == 1) {
      PROTECT(varlabels = allocVector(STRSXP, Ngram + 2 + f_count));
      pc++;
      for (i = 0; i < (Ngram + 2); i++) {
        if (i < Ngram) {
          ...
          // #if defined(WIN32)
          // 			SET_STRING_ELT(varlabels, i, mkCharCE( buf1,
          // CE_NATIVE )); #elif  defined(__MINGW32__)
          // SET_STRING_ELT(varlabels, i, mkCharCE( buf1,  CE_NATIVE )); #else
          // SET_STRING_ELT(varlabels, i, mkCharCE( buf1,  CE_UTF8 )); #endif

          SET_STRING_ELT(
              varlabels, i,
              mkCharCE(buf1, CE_UTF8));  // (utf8locale)?CE_UTF8:CE_NATIVE ));

        } else if (i == (Ngram)) {
          // #if defined(WIN32)
          // 			SET_STRING_ELT(varlabels, i, mkCharCE( "POS1",
          // CE_NATIVE )); #elif  defined(__MINGW32__)
          // SET_STRING_ELT(varlabels, i, mkCharCE( "POS1",  CE_NATIVE )); #else
          // SET_STRING_ELT(varlabels, i, mkCharCE( "POS1",  CE_UTF8 )); #endif

          SET_STRING_ELT(
              varlabels, i,
              mkCharCE("POS1", CE_UTF8));  // (utf8locale)?CE_UTF8:CE_NATIVE ));

        } else if (i == (Ngram + 1)) {
          // #if defined(WIN32)
          // 			SET_STRING_ELT(varlabels, i, mkCharCE( "POS2",
          // CE_NATIVE )); #elif  defined(__MINGW32__)
          // SET_STRING_ELT(varlabels, i, mkCharCE( "POS2",  CE_NATIVE )); #else
          // SET_STRING_ELT(varlabels, i, mkCharCE( "POS2",  CE_UTF8 )); #endif
          // 			//SET_STRING_ELT(varlabels, i, mkCharCE( "POS2",
          // CE_NATIVE ));

          SET_STRING_ELT(
              varlabels, i,
              mkCharCE("POS2", CE_UTF8));  // (utf8locale)?CE_UTF8:CE_NATIVE ));
        }
      }
    } else {  // if(NDF == 1)

      PROTECT(varlabels = allocVector(STRSXP, 3 + f_count));
      pc++;
      ...
      // SET_STRING_ELT(varlabels, 1, mkCharCE( "POS1", CE_NATIVE ));
      // SET_STRING_ELT(varlabels, 2, mkCharCE( "POS2",  CE_NATIVE
      // )); #else 		SET_STRING_ELT(varlabels, 0, mkCharCE( "TERM",
      // CE_UTF8 )); 		SET_STRING_ELT(varlabels, 1, mkCharCE( "POS1",
      // CE_UTF8
    ...
    if (mFt == 0 || mFt == 1) {  //  各ファイル名を列名として設定
      for (j = 0; j < f_count; j++) {
        sprintf(buf4, "%s",
                ff[j].c_str());  // 2011 03 10 sprintf(buf4, "%s", f[j]);//s

        if (NDF == 1) {  // Ngram 本体はいちいち単独列

          // #if defined(WIN32)
          // 			SET_STRING_ELT(varlabels, Ngram + 2 + j,
          // mkCharCE( buf4,  CE_NATIVE )); #elif  defined(__MINGW32__)
          // 			SET_STRING_ELT(varlabels, Ngram + 2 + j,
          // mkCharCE( buf4,  CE_NATIVE )); #else
          ...
          // #if defined(WIN32)
          // 			SET_STRING_ELT(varlabels, 3+ j, mkCharCE(buf4,
          // CE_NATIVE )); #elif  defined(__MINGW32__)
          // SET_STRING_ELT(varlabels, 3 +j, mkCharCE(buf4, CE_NATIVE )); #else
          // SET_STRING_ELT(varlabels, 3 + j, mkCharCE(buf4, CE_UTF8 )); #endif
      ...
      for (j = 0; j < f_count; j++) {
        ...
        if (NDF == 1) {                 // Ngram 本体はいちいち単独列
          ...
          // 			SET_STRING_ELT(varlabels, Ngram + 2 + j,
          // mkCharCE(buf4, CE_NATIVE )); #else
          // SET_STRING_ELT(varlabels, Ngram
          // + 2 + j, mkCharCE(buf4, CE_UTF8 )); #endif
  ...
  ///////////////////////// new_end
  // データフレームの行名を設定．必須
  PROTECT(row_names = allocVector(STRSXP, n0));
  pc++;

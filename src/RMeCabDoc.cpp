...
SEXP RMeCabDoc(SEXP filename, SEXP mypref, SEXP kigo, SEXP mydic) {
  ...
  if ((fp = fopen(input, "r")) == NULL) {
    ...
    PROTECT(my_list = allocVector(VECSXP,
                                  5000));  // 2019 06 10 2000 -> 5000 //pa++; 1
    // ファイル読み込み開始
      ...
      if (fgets(input2, FILEINPUT, fp) !=
        ...
        for (; node; node = node->next) {
          if (node->stat == MECAB_BOS_NODE)
            ...
            strcpy(
                buf2,
                node->feature);  // CSV で表記された形態素ごと(node)の素性情報
            if (strlen(buf2) < 1) {
              continue;
            }
            ...
            if (p != NULL) {
              ...
              //  // < 2005 11 08>
              // #if defined(WIN32)
              // 				SET_STRING_ELT(my_char2, 0,
              // mkCharCE(p, CE_NATIVE));  // 形態素情報をセット #elif
              // defined(__MINGW32__) 				SET_STRING_ELT(my_char2, 0, mkCharCE(p,
              // CE_NATIVE));  // 形態素情報をセット #else
              // 			 	SET_STRING_ELT(my_char2, 0,
              // mkCharCE(p, CE_UTF8));  // 形態素情報をセット #endif
              // 				//
              // SET_STRING_ELT(my_char2, 0, mkChar(p));  // 形態素情報をセット
              // // </ 2005 11 08>

              SET_STRING_ELT(my_char2, 0,
                             mkCharCE(p, CE_UTF8));  // (utf8locale)?CE_UTF8:CE_NATIVE));
                                                     // // 形態素情報をセット
              ...
              if (pref == 1) {  // 原型を取るため，六個目の要素を取る
                ...
                if (p == NULL || strcmp(p, "*") == 0) {  //
                  ...
                  // // < 2005 11 08>
                  // #if defined(WIN32)
                  // 					SET_STRING_ELT(my_char,
                  // 0, mkCharCE(buf1, CE_NATIVE)); #elif  defined(__MINGW32__)
                  // 					SET_STRING_ELT(my_char,
                  // 0, mkCharCE(buf1, CE_NATIVE)); #else
                  // 			 		SET_STRING_ELT(my_char,
                  // 0, mkCharCE(buf1, CE_UTF8)); #endif
                  // 					//
                  // SET_STRING_ELT(my_char, 0,
                  // mkChar(buf1));//buf1//入力語形をセット
                  //   // </ 2005 11 08>
                  ...
                  // #if defined(WIN32)
                  // 			 		SET_STRING_ELT(my_char,
                  // 0, mkCharCE(p, CE_NATIVE));//原型をセット #elif
                  // defined(__MINGW32__) 			 		SET_STRING_ELT(my_char, 0, mkCharCE(p,
                  // CE_NATIVE));//原型をセット #else 			 		SET_STRING_ELT(my_char, 0,
                  // mkCharCE(p, CE_UTF8));//原型をセット #endif
                  // 					//
                  ...
                  SET_STRING_ELT(
                      ...
                      mkCharCE(
                          p,
                          CE_UTF8));  // (utf8locale)?CE_UTF8:CE_NATIVE));//原型をセット

                } else {
                  //  // < 2005 11 08>
                  // #if defined(WIN32)
                  // 			 	SET_STRING_ELT(my_char, 0,
                  // mkCharCE(buf1, CE_NATIVE)); #elif  defined(__MINGW32__)
                  // 			 	SET_STRING_ELT(my_char, 0,
                  // mkCharCE(buf1, CE_NATIVE)); #else 			 	SET_STRING_ELT(my_char,
                  // 0, mkCharCE(buf1, CE_UTF8)); #endif
                  // 				//SET_STRING_ELT(my_char, 0,
                  // mkChar(buf1));
                  // 					// </ 2005 11 08>
              ...
              if (xlength(my_list) <=
                  (R_xlen_t)i) {  // 2015 12 28 if(length(my_list) <= i){
                UNPROTECT(1);
                PROTECT(my_list = lengthgets(
                            my_list,
                            i + 5000));  // pa++;  1 //2019 06 10 1000 -> 5000
              }
    ...
    UNPROTECT(1);                          // pa--;0
    if (xlength(my_list) > (R_xlen_t)i) {  // 2015 12 18 if(length(my_list) >
                                           // i){

...
NgramDF2 <-
  function(directory, type = 0, pos = "Default", minFreq = 1, N = 2, kigo = 0, dic = "", mecabrc = "", etc = "") {
    ...
    dtm <- .Call(NgramDF2_impl, as.character(directory), as.character(file), as.numeric(fileN), as.integer(ft), as.integer(type), as.character(pos), as.integer(posN), as.integer(minFreq), as.integer(N), as.numeric(kigo), as.character(opt), PACKAGE = "RMeCab")

      ...
      if (fgets(input2, FILEINPUT, fp) !=
        ...
        for (; node; node = node->next) {
          ...
          if (node->stat == MECAB_BOS_NODE)
            ...
            // 			  /////////	strncpy(buf2, node->feature,
            // node->length) ; 			  if(strlen(buf1) < 1){// 2006 06 移動 				continue;
            // 			  }
            ...
            // , CE_NATIVE  )); #elif  defined(__MINGW32__) 			  SET_STRING_ELT(
            // my_char  ,  0, mkCharCE( buf1 , CE_NATIVE )); #else
            // 			  SET_STRING_ELT(  my_char  , 0, mkCharCE(  buf1
            // , CE_UTF8)); #endif
            ...
            while (p != NULL && j < 10) {
              //  // < 2005 11 08>
              // #if defined(WIN32)
              // 			  SET_STRING_ELT( my_char   ,  j,
              // mkCharCE( p, CE_NATIVE  )); #elif  defined(__MINGW32__)
              // 			  SET_STRING_ELT(  my_char  ,  j,
              // mkCharCE( p , CE_NATIVE )); #else 			  SET_STRING_ELT(  my_char  ,
              // j, mkCharCE( p   , CE_UTF8)); #endif
              // 			  // SET_STRING_ELT(my_char, j,
              // mkChar(p));//元トークンを冒頭にセット
              // // </ 2005 11 08>
            ...
            pa--;  // pa-- 1;
            i++;
            if (i + 5000 > OVERLINE) {  // リストが10万(四万)を越えるならエラーを起こしやすい//
                                        // 2019 06 10 1000 -> 5000
              Rprintf("i == %d stop\n", i);
              fclose(fp);
              mecab_destroy(mecab);
